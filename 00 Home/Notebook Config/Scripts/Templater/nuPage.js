async function nuPage(tp, dv) {
    ...
    let noteDest;
    if (admin == true) {
        noteDest = await tp.system.suggester(["Inbox", "Current", "Root"], ["Inbox", "Current", "Root"], true, "Select Note Destination");
    } else if (admin == false) {
        noteDest = "Root"
    };
    ...
    if (noteDest !== "Inbox") {
        ...
        if (noteDest === "Current") {
            ...
            // noteFilter.selectCategories(dv, currentNote, currentCategories, currentCollections);
            // ({ inputSug, inputVal } =  noteFilter.createSuggesterInputs(dv, currentCollections, currentCollections, noteDest, admin));

        } else if (noteDest === "Root") {
            console.log("configCollections", configCollections);

            currentCollections = noteFilter.getChildNotesDev(dv, configCollections, "this", true, false)
            console.log("currentCollections", currentCollections);
            ({ inputSug, inputVal } = noteFilter.createSuggesterInputsDevII(dv, currentCollections, configCollections, noteDest, admin));
        ...
        let root = noteFilter.accessCatObject(collection, "noteType")[0]["path"].toLowerCase()
        ...
        while (true) {
            ...
            if (root.includes("collection")) {
                childNotesList = noteFilter.getChildNotesDev(dv, collection, "this", true, false);
            } else {
                childNotesList = noteFilter.getChildNotesDev(dv, collection, "this", true, null);
            }
            ...
            if (noteFilter.hasChildNotes(childNotesList)) {
                ...
                ({ inputSug, inputVal } = noteFilter.createSuggesterInputsDevII(dv, childNotesList, configCollections, "Selected", admin));
    ...
    console.log(noteFilter.accessCatObject(destinationNoteBook, "noteBook")[0].display)
    let NoteBookFile = noteFilter.accessCatObject(destinationNoteBook, "Page")[0]
    let NoteBookPath = noteFilter.accessCatObject(destinationNoteBook, "Path")[0]
    console.log("NoteBookPath", NoteBookPath, "NoteBookFile", NoteBookFile)
    let destNoteType = noteFilter.accessCatObject(destinationNoteBook, "noteType")[0];
    ...
    let folderStart = NoteBookFile.file.folder
    ...
    // Everything else is querying the folder of the template note and will generate a suggester if there is more than one
        ...
        if (fileTemplateNote.hasOwnProperty("folder") && Array.isArray(fileTemplateNote["folder"])) {
            ...
            targetFolder = String(noteFilter.accessCatObject(destinationNoteBook, "Folder")) + "/" + appendPath;
        } else {
           targetFolder = String(noteFilter.accessCatObject(destinationNoteBook, "Folder"));
        }
    }
    ...
    await app.fileManager.processFrontMatter(newTFile, frontmatter => {
        ...
        if (noteDest !== "Inbox"){
            let destinationNoteTypeName = destNoteType.display
            // frontmatter[destinationNoteTypeName] = "[[" + String(noteFilter.accessCatObject(destinationNoteBook, "Path")).split(".md")[0] + 
            // "|" + noteFilter.accessCatObject(destinationNoteBook, "Name") + "]]";
            // if (templateType == "Category") {
                // frontmatter[destinationNoteTypeName] = "[[" + newDVFile.file.path.split(".md")[0] +
                //  "|" + newDVFile.file.name + "]]";
                frontmatter["parent"] = "[[" + String(noteFilter.accessCatObject(destinationNoteBook, "Path")).split(".md")[0] +
                 "|" + noteFilter.accessCatObject(destinationNoteBook, "Name") + "]]"; 
                frontmatter["noteBook"] = "[[" + noteFilter.accessCatObject(destinationNoteBook, "noteBook")[0].path.split(".md")[0] +
                 "|" + noteFilter.accessCatObject(destinationNoteBook, "noteBook")[0].display + "]]";
                // Cleaning up Category Template frontmatter
                delete frontmatter["templateType"];
                delete frontmatter["childTemplate"];
                delete frontmatter["rootFolder"];
                delete frontmatter["rootNote"];
